...
export function getGitBranch(cwd) {
    try {
        return execFileSync("git", ["branch", "--show-current"], { cwd, timeout: 2000, encoding: "utf-8" }).trim() || null;
    }
    catch {
        return null;
    }
...
export function getRepository(cwd) {
    try {
        const remoteUrl = execFileSync("git", ["config", "--get", "remote.origin.url"], { cwd, timeout: 2000, encoding: "utf-8" }).trim();
        if (!remoteUrl) {
            return null;
        }
